...
{
    try {
        App.games = await apiRequest('/api/games');   // добавлен /api
    } catch (error) {
        App.games = [];
    }
}
...
async function startNewGame()
{
    ...
    try {
        const result = await apiRequest('/api/games', 'POST', { player_name: playerName });
...
{
    const progressionHtml = App.currentStep.progressionArray.map((value) => {
...
async function submitAnswer()
{
    ...
    try {
        const result = await apiRequest(`/api/step/${App.currentGame.id}`, 'POST', {
